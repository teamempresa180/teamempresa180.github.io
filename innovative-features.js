document.addEventListener('DOMContentLoaded', () => {
    ...
    // Desactivado para los h2 a petición del usuario
    const scrambleLetters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789@#$%&*<>/|";
    const scrambleElements = document.querySelectorAll('.hero-label'); // Eliminado h2:not(.hero-headline)
        ...
        el.addEventListener('mouseenter', event => {
            ...
            el.scrambleInterval = setInterval(() => {
                event.target.innerText = event.target.innerText.split("").map((letter, index) => {
                    ...
                    if(event.target.dataset.original[index] === ' ') {
                        return ' ';
                    }
                ...
                iterations += 1; // Speed of scramble
            }, 30);
