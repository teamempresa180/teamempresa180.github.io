...
new_admin_content = r"""<!DOCTYPE html>
<html lang="es">
<head>
  <meta charset="UTF-8">
...
<div id="login-overlay">
  <div class="login-box">
    ...
    <button class="btn btn-primary" style="width:300px;" onclick="login()">Acceder al Sistema</button>
    <p id="login-error" style="color:#f87171;margin-top:12px;display:none;">Contraseña incorrecta.</p>
    <p style="font-size:.78rem;color:#444;margin-top:20px;">Pista: 123456789</p>
  </div>
</div>
...
<div class="dashboard" id="dashboard">
  ...
  <aside class="sidebar">
    ...
    <div class="nav-item" onclick="window.location.href='index.php'" style="margin-top:auto;">
...
const FIELDS = ['name','email','phone','company','source','budget','status','message'];
...
//  INIT
// ═══════════════════════════════════════════════
function initApp() { showSkeletons(); loadLeads(); }
...
function renderTable(leads) {
  ...
  tbody.innerHTML = leads.map(l => {
    ...
    return `<tr>
      ...
      <td style="color:var(--muted);font-size:.85rem">${esc(l.source||'—')}</td>
  ...
  try {
    if(editingId) {
      ...
      if(d.success) {
        ...
        if(i>-1) allLeads[i] = {...allLeads[i],...payload, score: d.score};
